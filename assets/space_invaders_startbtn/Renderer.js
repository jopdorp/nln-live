...
(function () {
	...
	Renderer.prototype.addBlock = function (block)
	{
		this._bunkers[block.id] = block;
	};
	
	Renderer.prototype.removeBlock = function (block)
	{
		delete this._bunkers[block.id];
	};
...
// private
    ...
    function draw ()
    {
		...
		for (var id in this._enemies) {
			...
			this._context.save();
			if(enemy._blinking == true) {
				this._context.drawImage(enemy.imgSrcW, enemy.x, enemy.y); }
		...
		for (var id in this._player) {
			...
			if(player._frame == 1) {
				...
				this._context.drawImage(player.imgBlinkSrc, player.x, player.y); }
			
			for (i=0; i<= player._lives; i++){
				if(player._frame == 1) {
					this._context.drawImage(player.imgSrc, (this._width / 2) + 180 + (i * 38), 10); }
				else {
					this._context.drawImage(player.imgBlinkSrc, (this._width / 2) + 180 + (i * 38), 10); }
			}
			
			this._context.restore();
		}
		
		for (var id in this._bunkers) {
			var bunker = this._bunkers[id];
			this._context.save();

			this._context.drawImage(bunker.imgSrc, bunker.x, bunker.y);
